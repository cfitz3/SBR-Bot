...
- **In-game playtime is measured, not sampled.** The bridge opens a `PlaySession` on an observed join and closes it on the leave, so hours are real elapsed time rather than a sample count multiplied by a cadence. `ActivityDaily.presenceSamples` is left in place because every historical XP award was computed from it, but it is no longer the source for anything new. Discord-side presence is still unsampled — `XpService.recordPresence` exists and has no caller.
...
- **Message counting is independent of the XP policy.** A guild with no XP sources configured still accrues `ActivityDaily` counters — an unconfigured source counts any non-empty message on a fixed 5s spacing, and only an *explicitly disabled* source stops counting. It used to be gated on the policy, which meant every install that had not configured XP saw an Analytics page of zeros while the relay was busy. The anti-farming limits still bite where they belong, at the award end: an unconfigured source is worth zero XP, so the counters move and nobody's standing does. See `DOMAIN_MODEL.md` → ActivityDaily.
