...
**The look is the Operator design system** (`public/app.css`): a single dark palette — ground `#0d0f12`, surface `#101318`, a cold blue accent `#5fa8d3` — set in IBM Plex Sans where it is installed, over IBM Plex Mono for anything machine-read. Three rules govern the whole sheet, and every local decision follows from one of them:

- **Colour means state.** The accent marks the *selected* thing and nothing else — the active nav row, the current tab, the primary button. Nothing decorative is tinted, which is what makes a tinted thing findable without hunting. Status hues (`ok`/`warn`/`bad`) are the only other coloured text, and they appear as bare mono words with a leading dot rather than as filled pills.
- **A hairline is the separator.** Grids get their rules from `gap: 1px` over a dark ground, so the divider *is* the gap rather than a border drawn near one; tables rule their rows the same way. There are no shadows and no backdrop blur — the three `shadow` tokens are 1px rings, kept under their old names so an override written against them still lands.
- **Mono is for machine-read text.** Numerals, IGNs, ids, timestamps, counts, status words and small uppercase labels are all `--font-mono` with `tabular-nums`, so a column of figures reads as a column.

Three things in the source design could not be ported literally, all for the same reason — the CSP has no `'unsafe-inline'` and no off-origin host:
...
- **The webfont became a font stack.** IBM Plex Sans and IBM Plex Mono are named first and used if the operator has them; the system stacks carry the same measurements otherwise.
...
The chrome is a 206px sticky sidebar rather than a top tab strip: eleven pages do not fit across a laptop's width without wrapping, and the sidebar has room to group them under **Monitor**, **Queues** and **Configure** — watching, then the queues that need a person, then the settings that change how the platform behaves. Below 900px the whole thing stacks and the nav becomes a wrapped row, so every page stays reachable on a phone. The design's own light variant is not implemented and is not planned; a light Operator would be a second design, not a second theme.
...
- **The same hop also gives the panel a voice.** The panel holds no gateway connection, so a moderation action taken here had no way to post its mod-log card and, until this was wired, posted none — see `ADMIN_BOT.md` §7b (`announce`) and `MODERATION_AUDIT_LOG.md` fourth pass. The panel decides what to say and which slot it belongs in (`modlog`, falling back to `staff`); the admin bot performs the write, as it does for every other privileged action.
