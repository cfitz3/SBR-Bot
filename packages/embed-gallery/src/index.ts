...
export const GALLERY: readonly GalleryCard[] = [
  ...
  card("whois", "`/whois` — a member with more roles than fit.", renderWhoisEmbed, f.DISCORD_USER),
  card(
    "whois-outsider",
  ...
  card("event-signup", "The event message when it is posted — the roster, and the buttons that join it.", renderEventCard, f.EVENT_SIGNUP),
  card("event-board", "The same message once the event goes live: the roster gives way to the table.", renderEventCard, f.EVENT_BOARD),
  card("event-board-final", "The same message again, one last edit into the result left in the channel.", renderEventCard, f.EVENT_BOARD_FINAL),
  card("attendance", "Who turned up, then who said they were coming.", renderAttendanceEmbed, f.ATTENDANCE),
  card("lfg-request", "`/lfg` — one member asking for a group.", renderLfgRequestCard, f.LFG_REQUEST),
  card(
    "lfg-request-bare",
    "The same ask with the numbers unread and no class named — the fields drop rather than printing unknowns.",
    renderLfgRequestCard,
    f.LFG_REQUEST_BARE,
  ),
  ...
  card("perm", "`/perm` — a standing party and its roster.", renderPermCard, f.PERM),
  ...paged("perm-list", "`/perms` — every party, disbanded included.", renderPermListPages, f.PERMS, false),
  ...paged("perm-list-mine", "The same list addressed to its owner.", renderPermListPages, f.PERMS, true),
