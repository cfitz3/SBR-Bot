...
export const DEFAULT_PANEL = {
  ...
  milestones: {
    ...
    metric: {
      ...
      fairySouls: "Fairy souls found",
      museumDonations: "Museum donations",
      petScore: "Pet score",
      minionSlots: "Minion slots",
      essence: "Essence held, all types",
    ...
    /* ── What /progression offers ── */
    offeredTitle: "Charted metrics",
    offeredNote:
      "Which numbers /progression puts on its menu. The tracker records every metric either way — a reading " +
      "not taken is history that cannot be recovered later — so narrowing this only changes what members are " +
      "offered, never what is kept.",
    /** `{n}` of `{limit}` chosen; the limit is Discord's cap on a menu. */
    offeredCount: "{n} of {limit} chosen",
    offeredEmpty: "Pick at least one — an empty menu is not a configuration.",
    offeredFull: "At most {limit} — Discord will not render a longer menu.",
    offeredSave: "Save charted metrics",
  ...
  settings: {
    ...
    /** `{keys}` are stored flags this build has no reader for. */
    featureStale: "Stored but unused on this build: {keys}. Nothing reads these; they can be ignored.",
